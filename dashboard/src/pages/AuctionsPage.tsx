    ...
    try {
      ...
      const transformedAuctions = data.map((auction: any, index: number) => {
        ...
        const winner = highestBid ? (
          ...
          (highestBid.Bidder ? `${highestBid.Bidder.FirstName} ${highestBid.Bidder.LastName}` :
          (highestBid.bidder ? `${highestBid.bidder.firstName || highestBid.bidder.FirstName} ${highestBid.bidder.lastName || highestBid.bidder.LastName}` : null))
        ) : null;
                          ...
                          {auctionBids.map((bid: any, index: number) => (
                            <tr key={bid.bidId || bid.BidId || index} style={{ 
                              ...
                              <td style={{ padding: '0.75rem', fontSize: '0.75rem', color: '#6b7280' }}>
                                {bid.Bidder?.Email || bid.bidder?.email || bid.bidder?.Email || bid.bidderEmail || '-'}
                              </td>
